	...
	<cftry>
		<!--- Encrypt the value. --->
		<cfset strRememberLogin = decrypt(
			cookie.RememberMe,
		...
		<cfset strUsername=trim(listgetat(strRememberLogin,2,":"))>
		<cfset strPassword=trim(listgetat(strRememberLogin,3,":"))>
		<cfset strloginLoc=trim(listgetat(strRememberLogin,4,":"))>
		<cfif strloginLoc EQ "6FJ" OR strloginLoc EQ "L365">
			<cfset session.EXEC.strloginLoc="#strloginLoc#">
		<cfelse>
			<cfset session.EXEC.strloginLoc="6FJ">
		</cfif>
		
		<cfset blnRememberMe =1>
