...
<cfif not len(intResID)>
	<cflocation url="/member-logout">
</cfif>
<cfset blnAlertOn=0>
<cfset blnAlertOn=0>
<cfparam name="pageLink" default="/member-dashboard" />
<cfif isdefined("session.EXEC.dteSubmitted") and session.EXEC.dteSubmitted gt "10/23/2007">
	
	<cfset encryptedResID=encrypt(intResID, 'VhnSz48Nx1IN8IzaXY0WVQ==', 'AES', 'Base64') />
...
<cfif ((lcl_intResAdmCode eq 1) OR (lcl_intResAdmCode eq 2) OR (lcl_intResAdmCode eq 20))> <!--- incomplete or "in-review" profile --->
	<cfif (lcl_intResAdmCode eq 1)>
		...
		<div class=" row">
			...
			<br><br>
			<a href="#pageLink#">Click here to complete the registration process</a>
			<br>
			</div>
		</div>
	<!--- new member message --->
	...
	<cfif DateDiff('d',resumedate,now) gte 90>
		<div class="alert alert-info">
			...
			<div style="font-size:13px; margin: 5px 0px 5px 0px; padding:5px 0px 5px 0px; font-weight:bold;">
				...
				<div style="padding-bottom:8px;"></div>
				<a href="/member-refresh" title="Refresh your resume" role="button" class="btn btn-primary btn-small">Refresh Resume</a>
				<br clear="all"/>
